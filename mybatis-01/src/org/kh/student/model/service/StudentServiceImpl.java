...
public class StudentServiceImpl implements StudentService {
	...
	public int insertStudent() {
		...
		//getSqlSession 메소드를 통해 SqlSession값을 리턴
	...
	public SqlSession getSqlSession() { //기존 getConnection 과 같음
		//getSqlSesstion 메소드의 역할은
		//DB에 연결할 JDBC 연결 코드를 통하여 연결하는 세션 정보를
		//리턴해주는 역할
		
		//Mybatis에서 SqlSession 만들기 위한 순서(SqlSession==Connection 객체)
		//1.드라이버 및 JDBC연결 정보를 가지고 있는 xml파일을 읽어 들여야 함(InputStream 생성)
		//2.SqlSession을 만들려면 SqlSessionFactory 객체가 필요함
		//SqlSessionFactory 객체를 만들려면 SqlSessionFactoryBuilder 객체가 있어야함
		...
		try {
			//1.드라이버 및 JDBC 연결 정보를 가지고 있는 xml파일을 읽어 들여야 함(InputStream 생성)
...
//Mybatis 또한 같음
//하지만! 드라이버 등록 코드 및 연결할 주소 등록 코드를xml 파일에 
//미리 등록하고 가져다 사용하는 원리
